...
\KwIn { Coefficient matrix $A$ \\
\hspace{0.41in} Point matrix $S$\\
\hspace{0.41in} Measure Data $D$
}
\KwOut {Acoustic source location}
...
\hspace{0.41in} Point matrix $S$\\
\hspace{0.41in} Measure Data $D$\\
\hspace{0.41in} Maximum number of iterations $maxiter$
}
\KwOut {Acoustic source location}
...
\For{$i \leftarrow 1$ \textbf{to} $maxiter$}
{
     $\varDelta=sign(ASX)-D$;\\
     $\bigtriangledown=sign((AS)'*\varDelta)$;\\
     $X=X-\mu*\bigtriangledown$;\\
